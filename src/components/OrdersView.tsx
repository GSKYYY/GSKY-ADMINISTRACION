...
import { Search, Plus, Calendar, Filter, X, Printer, Image as ImageIcon, Shirt, User, Package, Clock, Ruler, ChevronDown, CheckCircle2, AlertTriangle, Edit, DollarSign, FileText, Layers, CloudUpload, Eye, Pencil, CalendarDays, Trash2, Feather, Briefcase, MousePointer2, Tag, Download, Loader2, Crown, Save, UserPlus, Phone, MapPin, Scissors, Zap, ToggleRight, ToggleLeft, Lock, ArchiveRestore, ArrowRight } from 'lucide-react';
...
const checkCategory = (o: Order, cat: FilterCategory): boolean => {
    ...
    const isEmbroidery = EMBROIDERY_OPTIONS.includes(o.garmentModel) || safeItems.some(i => i.type && i.type.toLowerCase().includes('bordado'));
    const isSublimation = SUBLIMATION_OPTIONS.includes(o.garmentModel) || safeItems.some(i => i.type && i.type.toLowerCase().includes('sublima'));
    const isSewing = SEWING_OPTIONS.includes(o.garmentModel) || safeItems.some(i => i.type && (i.type.toLowerCase().includes('ruedo') || i.type.toLowerCase().includes('costura') || i.type.toLowerCase().includes('ajuste')));
...
const PrintTemplate = ({ order }: { order: Order }) => {
    ...
    const isStandardClothing = safeItems.some(i => GENDER_ORDER.includes(i.gender));
    ...
    const maleItems = safeItems.filter(i => i.gender === 'Caballero');
    const boyItems = safeItems.filter(i => i.gender === 'Niño');
    const hasMale = maleItems.length > 0;
    const hasBoy = boyItems.length > 0;
    
    const femaleItems = safeItems.filter(i => i.gender === 'Dama');
    const girlItems = safeItems.filter(i => i.gender === 'Niña');
        ...
        >
                    ...
                    {isStandardClothing ? (
                        <div style={{ display: 'flex', gap: '15px' }}>
                            <div style={{ flex: 1, border: '1px solid black' }}>
                                ...
                                {safeItems.filter(i => i.gender === 'Caballero' || i.gender === 'Niño').length > 0 ? (
                                    safeItems.filter(i => i.gender === 'Caballero' || i.gender === 'Niño')
                            ...
                            <div style={{ flex: 1, border: '1px solid black' }}>
                                ...
                                {safeItems.filter(i => i.gender === 'Dama' || i.gender === 'Niña').length > 0 ? (
                                    safeItems.filter(i => i.gender === 'Dama' || i.gender === 'Niña')
                                    .sort((a,b) => getSizeWeight(a.size) - getSizeWeight(b.size))
                                    .map((item, idx) => (
                                        <div key={idx} style={{ display: 'flex', borderBottom: '1px solid #ccc', fontSize: '10pt' }}>
                                            <div style={{ flex: 1, padding: '4px', borderRight: '1px solid #ccc', fontWeight: 'bold', textAlign: 'center' }}>
                        ...
                        <div style={{ border: '1px solid black' }}>
                            ...
                            {safeItems.map((item, idx) => (
                                <div key={idx} style={{ display: 'flex', borderBottom: idx === safeItems.length - 1 ? 'none' : '1px solid #ccc', fontSize: '10pt' }}>
            ...
            <div style={{ position: 'absolute', bottom: '2cm', right: '1.5cm', textAlign: 'right' }}>
                <div style={{ borderTop: '2px solid black', paddingTop: '5px' }}>
                    <span style={{ fontSize: '14pt', fontWeight: 'bold' }}>TOTAL PIEZAS: {safeItems.reduce((a,b)=>a+b.quantity,0)}</span>
                </div>
            </div>
        </div>
    );
...
export const OrdersView: React.FC<OrdersViewProps> = ({ orders, clients, onAddOrder, onUpdateOrder, onUpdateStatus, onAddClient }) => {
  ...
  // --- NEW QUICK CLIENT STATES ---
  const [isQuickClientOpen, setIsQuickClientOpen] = useState(false);
  const [quickClientForm, setQuickClientForm] = useState({ name: '', businessName: '', phone: '', address: '' });
  ...
  // --- HANDLER: Quick Client Save ---
  const handleQuickClientSave = () => {
      if (!quickClientForm.name) return alert("El nombre de contacto es obligatorio.");
      ...
      const newClient: Client = {
          ...
          businessName: quickClientForm.businessName || '',
          phone: quickClientForm.phone || '', // Optional
      ...
      setNewOrder(prev => ({ ...prev, clientId: newClientId }));
      setClientSearch('');
      setIsQuickClientOpen(false);
      setQuickClientForm({ name: '', businessName: '', phone: '', address: '' });
  };

  const handleAddItem = () => {
    if (!isReadyForItems) return;
    if (tempQty <= 0) return;

    let finalSize = '';
    let itemGender: Gender = 'Caballero';
    let itemNote = '';
  ...
  const handleSaveOrder = (e: React.FormEvent) => {
    ...
    const order: Order = {
      ...
      orderNumber: orderNumberInput, // Use manual input
  ...
  return (
    <div className="space-y-8 animate-enter h-full">
      ...
      {/* QUICK CLIENT MODAL */}
      {isQuickClientOpen && (
          <div className="fixed inset-0 z-[1200] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-enter">
              <div className="bg-[#0f0a29] border border-white/20 rounded-3xl p-8 w-full max-w-md shadow-2xl relative">
                   <button onClick={() => setIsQuickClientOpen(false)} className="absolute top-4 right-4 text-slate-400 hover:text-white"><X size={20}/></button>
                   <h3 className="text-xl font-display font-black text-white mb-6 flex items-center gap-2">
                       <UserPlus className="text-primary-400"/> Creación Rápida
                   </h3>
                   <div className="space-y-4">
                       <div>
                           <label className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1.5 block">Nombre del Negocio</label>
                           <input autoFocus type="text" className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-white focus:border-primary-500 outline-none" placeholder="Ej. Taller El Éxito" value={quickClientForm.businessName} onChange={e => setQuickClientForm({...quickClientForm, businessName: e.target.value})} />
                       </div>
                       <div>
                           <label className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1.5 block">Nombre Contacto *</label>
                           <input type="text" className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-white focus:border-primary-500 outline-none" placeholder="Ej. Juan Pérez" value={quickClientForm.name} onChange={e => setQuickClientForm({...quickClientForm, name: e.target.value})} />
                       </div>
                       <div>
                           <label className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1.5 block">Teléfono (Opcional)</label>
                           <input type="tel" className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-white focus:border-primary-500 outline-none" placeholder="555-0000" value={quickClientForm.phone} onChange={e => setQuickClientForm({...quickClientForm, phone: e.target.value})} />
                       </div>
                       <div className="pt-4">
                           <button onClick={handleQuickClientSave} className="w-full py-4 bg-primary-600 hover:bg-primary-500 text-white font-bold rounded-xl shadow-lg shadow-primary-900/40 transition-all uppercase tracking-widest text-sm">Guardar y Seleccionar</button>
                       </div>
                   </div>
              </div>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-2 gap-6 print:hidden pb-24">
         {filteredOrders.length > 0 ? filteredOrders.map(order => {
             ...
             const getCardBg = () => {
                 ...
                 if (order.status === OrderStatus.TRASH) return 'bg-[#101015] border-slate-800';
                 return 'bg-[#18122B] border-white/10';
             };
                 ...
                 <div key={order.id} className={`relative overflow-visible flex flex-col h-full ${getCardBg()} border rounded-[2.5rem] shadow-xl hover:shadow-2xl hover:scale-[1.01] transition-all duration-300 group cursor-default z-0`}>
                     ...
                     <div className="flex flex-col h-full p-6 pl-8">
                        <div className="flex justify-between items-start mb-5 relative z-20">
                            <span className="bg-white/5 border border-white/10 px-3 py-1.5 rounded-lg text-slate-300 font-mono text-xs font-bold tracking-wider shadow-sm">
                                {order.orderNumber}
                            </span>
                        ...
                        <div className="mb-6 relative z-0">
                            <h3 className="font-display font-black text-3xl text-white leading-none tracking-tight mb-2 group-hover:text-primary-300 transition-colors uppercase line-clamp-1 drop-shadow-md">{order.clientName}</h3>
                        ...
                        <div className="grid grid-cols-2 gap-3 mb-6 flex-1">
                             <div className="p-3.5 rounded-2xl bg-blue-500/5 border border-blue-500/10 flex flex-col justify-center">
                             ...
                             <div className="p-3.5 rounded-2xl bg-purple-500/5 border border-purple-500/10 flex flex-col justify-center">
                        ...
                        <div className="mt-auto pt-4 border-t border-white/10 flex items-center justify-between">
                            <div className={`flex items-center gap-3 ${isOverdue ? 'text-red-400 animate-pulse' : 'text-slate-300'}`}>
                        ...
                        <div className="mt-5 grid grid-cols-5 gap-2">
                             ...
                             <button onClick={() => handleEditClick(order)} className="col-span-2 py-3 bg-indigo-500/10 hover:bg-indigo-500/20 border border-indigo-500/20 rounded-xl text-xs font-bold text-indigo-300 hover:text-white uppercase tracking-wider transition-colors flex items-center justify-center gap-2">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/90 p-2 md:p-6 backdrop-blur-2xl">
            <div className="bg-[#050314] w-full max-w-[95vw] h-full md:h-[90vh] md:rounded-[2.5rem] shadow-2xl flex flex-col relative overflow-hidden animate-enter border border-white/10">
                <div className="flex justify-between items-center px-4 md:px-8 py-4 md:py-6 border-b border-white/5 bg-[#0a0520] relative z-20 shrink-0">
                    <div>
                        <h3 className="text-2xl md:text-3xl lg:text-4xl font-display font-black text-white tracking-tight">
                        ...
                        <div className="flex items-center gap-2 mt-2">
                             <label className="text-xs text-slate-500 font-bold uppercase tracking-wider">Nro. Pedido:</label>
                             <input 
                                type="text" 
                                value={orderNumberInput}
                                onChange={(e) => setOrderNumberInput(e.target.value)}
                                className="bg-transparent border-b border-white/20 text-white font-mono font-bold text-sm outline-none w-24 focus:border-primary-500 transition-colors"
                             />
                        </div>
                    </div>
                    <button onClick={() => { setIsModalOpen(false); resetForm(); }} className="text-slate-400 hover:text-white p-2 md:p-3 hover:bg-white/10 rounded-full transition-colors"><X size={28} /></button>
                </div>
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="space-y-6">
                                    <div className="relative group">
                                         {newOrder.clientId ? (
                                            <div className="bg-primary-900/10 border border-primary-500/40 p-4 rounded-2xl flex items-center justify-between animate-enter">
                                    ...
                                    <div className="bg-black/40 p-4 rounded-2xl border border-white/10">
                                        <div className="flex justify-between items-center mb-3">
                                            ...
                                            <div className="flex items-center gap-2">
                                                <span className="text-xs text-slate-500 font-bold uppercase">Hora?</span>
                                                <button onClick={() => setUseSpecificTime(!useSpecificTime)} className={`text-2xl transition-colors ${useSpecificTime ? 'text-primary-400' : 'text-slate-600'}`}>{useSpecificTime ? <ToggleRight size={28}/> : <ToggleLeft size={28}/>}</button>
                                            </div>
                                        </div>
      ...
      {isDetailOpen && selectedOrder && (
          <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/80 p-0 md:p-6 backdrop-blur-xl">
              <div className="bg-[#0a0520] md:rounded-3xl w-full max-w-7xl shadow-2xl flex flex-col h-full md:h-[95vh] relative animate-enter overflow-hidden">
                  ...
                  <div className="flex justify-between items-center px-4 md:px-8 py-5 border-b border-white/10 bg-[#0f0a29] shrink-0 print:hidden">
                      <div className="flex items-center gap-5"><div className="p-3 bg-white/5 rounded-2xl border border-white/10"><FileText className="text-primary-400" size={24}/></div><div><h2 className="text-2xl md:text-3xl font-display font-black text-white tracking-wide">{selectedOrder.orderNumber}</h2><span className={`text-xs font-bold px-3 py-1 rounded-md border ${getStatusColor(selectedOrder.status)}`}>{selectedOrder.status}</span></div></div>
                      <div className="flex gap-3">
                         <button onClick={() => { setIsDetailOpen(false); handleEditClick(selectedOrder); }} className="bg-indigo-600/20 border border-indigo-600/40 text-indigo-300 px-5 py-3 rounded-xl flex items-center gap-2 hover:bg-indigo-600/30 font-bold uppercase transition-all text-xs shadow-lg"><Edit size={18} /> <span className="hidden md:inline">Editar</span></button>
                         <button onClick={handleDownloadPdf} disabled={isGeneratingPdf} className="bg-emerald-500/20 border border-emerald-500/40 text-emerald-300 px-5 py-3 rounded-xl flex items-center gap-2 hover:bg-emerald-500/30 font-bold uppercase transition-all text-xs shadow-lg disabled:opacity-50">{isGeneratingPdf ? <Loader2 size={18} className="animate-spin"/> : <Download size={18} />} <span className="hidden md:inline">{isGeneratingPdf ? 'Generando...' : 'PDF'}</span></button>
                         <button onClick={() => window.print()} className="bg-white text-black px-5 py-3 rounded-xl flex items-center gap-2 hover:bg-gray-200 font-bold uppercase transition-transform text-xs shadow-lg"><Printer size={18} /> <span className="hidden md:inline">Imprimir</span></button>
