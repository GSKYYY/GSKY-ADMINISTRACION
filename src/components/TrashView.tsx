...
import { Trash2, ArchiveRestore, AlertTriangle, XCircle, Search } from 'lucide-react';
          ...
          {trashOrders.map(order => (
            <div key={order.id} className="bg-[#150a25] border border-white/5 rounded-3xl p-6 relative group hover:border-red-500/30 transition-all hover:-translate-y-1 shadow-lg">
                ...
                <div className="grid grid-cols-2 gap-3 mt-auto">
                    <button 
                        onClick={() => { if(confirm('¿Restaurar este pedido a Recibido?')) onRestore(order.id); }}
                    ...
                    <button 
                        onClick={() => { if(confirm('⚠️ ¿ESTÁS SEGURO? Esta acción borrará el pedido permanentemente.')) onDeletePermanently(order.id); }}
