...
export const ClientsView: React.FC<ClientsViewProps> = ({ clients, orders, onAddClient, onUpdateClient, onDeleteClient }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.name) return alert('El nombre de contacto es obligatorio');
    ...
    if (selectedClient && isFormModalOpen) {
        onUpdateClient({ ...selectedClient, ...finalData } as Client);
    } else {
        onAddClient({ id: crypto.randomUUID(), createdAt: Date.now(), ...finalData } as Client);
    }
          ...
          >
              ...
              <div className="flex items-start gap-5 mb-6">
                  ...
                  <div className="min-w-0 flex-1 pt-1">
                      ...
                      <p className="text-xs text-slate-500 font-mono mt-1">{client.phone || 'Sin teléfono'}</p>
                  </div>
              </div>
                                ...
                                <div className="w-full md:w-64 flex flex-col gap-3 shrink-0">
                                    ...
                                    <div className="flex-1 overflow-y-auto space-y-2 pr-1 custom-scrollbar max-h-[300px] md:max-h-none">
                                        {personnelList.map(person => (
                                            <button key={person.id} onClick={() => setActivePersonId(person.id)} className={`w-full p-4 rounded-xl border text-left transition-all relative group ${activePersonId === person.id ? 'bg-primary-600/20 border-primary-500 text-white shadow-[0_0_15px_rgba(217,70,239,0.2)]' : 'bg-black/30 border-white/5 text-slate-400 hover:bg-white/5 hover:text-white'}`}>
                                                ...
                                                <div onClick={(e) => { e.stopPropagation(); handleRemovePerson(person.id); }} className="absolute right-2 top-1/2 -translate-y-1/2 p-2 hover:bg-red-500/20 text-slate-500 hover:text-red-400 rounded-lg transition-colors"><Trash2 size={14}/></div>
                                            </button>
                                        ))}
                                    </div>
                                </div>
                                    ...
                                    {activePerson ? (
                                        <div className="space-y-6 animate-enter">
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                                <div className="space-y-2">
                                                    ...
                                                    <div className="relative">
                                                        ...
                                                        <input type="text" className="w-full pl-12 pr-4 py-4 bg-black/40 border border-white/10 rounded-2xl text-white font-bold text-lg focus:border-primary-500 focus:bg-black/60 outline-none transition-colors" placeholder="Ej. Juan Pérez - Gerente" value={activePerson.name} onChange={(e) => updatePersonField(activePerson.id, 'name', e.target.value)}/>
                                                    </div>
                                                </div>
                                                <div className="space-y-2">
                                                    ...
                                                    <div className="relative">
                                                        ...
                                                        <select className="w-full pl-12 pr-4 py-4 bg-black/40 border border-white/10 rounded-2xl text-white font-bold text-lg focus:border-primary-500 focus:bg-black/60 outline-none transition-colors appearance-none" value={activePerson.standardSize} onChange={(e) => updatePersonField(activePerson.id, 'standardSize', e.target.value)}>
                                                ...
                                                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                                                    {MEASUREMENT_FIELDS.map((field) => (
                                                        <div key={field.key} className="space-y-1">
                                                            ...
                                                            <input type="text" className="w-full p-3 bg-black/20 border border-white/5 rounded-xl text-white font-mono font-medium focus:border-primary-500 focus:bg-black/40 outline-none transition-colors text-center" placeholder="-" value={activePerson.values[field.key] || ''} onChange={(e) => updatePersonValue(activePerson.id, field.key, e.target.value)}/>
                                                        </div>
                                                    ))}
                                                </div>
                                            </div>
                                            <div className="space-y-2">
                                                 ...
                                                 <textarea className="w-full p-4 bg-black/40 border border-white/10 rounded-2xl text-white font-medium focus:border-primary-500 outline-none transition-colors h-24 resize-none" placeholder="Detalles sobre postura, hombros caídos..." value={activePerson.notes} onChange={(e) => updatePersonField(activePerson.id, 'notes', e.target.value)}/>
                                            </div>
                                        </div>
                                    ) : (
                                        <div className="h-full flex flex-col items-center justify-center text-slate-500 opacity-60"><Users size={48} className="mb-4"/><p className="text-lg font-bold">Selecciona una persona</p></div>
                                    )}
                                </div>
                            </div>
                        </div>
                            ...
                            {clientOrders.length > 0 ? (
                                clientOrders.map(order => (
                                    <div key={order.id} className="glass-card p-5 rounded-2xl border border-white/10 flex flex-col md:flex-row items-center gap-6 hover:bg-white/5 transition-colors">
                                        <div className="flex flex-col items-center md:items-start min-w-[100px]"><span className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Fecha</span><span className="text-white font-mono font-bold">{new Date(order.createdAt).toLocaleDateString()}</span></div>
                                        <div className="flex-1 text-center md:text-left"><span className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Pedido</span><div className="flex items-center justify-center md:justify-start gap-3"><span className="bg-white/10 px-2 py-0.5 rounded text-xs font-mono text-white">{order.orderNumber}</span><span className="text-white font-bold">{order.garmentModel}</span></div></div>
                                        <div className="text-center md:text-right min-w-[100px]"><span className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Monto</span><span className="block text-emerald-400 font-black text-lg">${order.totalAmount}</span></div>
                                        <div className="min-w-[120px] flex justify-center"><span className={`px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest border ${order.status === 'Entregado' ? 'bg-emerald-500/10 border-emerald-500 text-emerald-400' : 'bg-blue-500/10 border-blue-500 text-blue-400'}`}>{order.status}</span></div>
                                    </div>
                                ))
                            ) : (
                                <div className="text-center py-20 opacity-50"><ShoppingBag size={48} className="mx-auto mb-4 text-slate-500"/><h4 className="text-xl font-bold text-white">Sin historial</h4></div>
                            )}
                        </div>
                    )}
                </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 backdrop-blur-xl p-4 animate-enter">
          <div className="bg-[#0a0520] border border-white/10 rounded-[2.5rem] w-full max-w-lg shadow-2xl relative overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <form onSubmit={handleSubmit} className="p-8 md:p-10 space-y-6 md:space-y-8 relative z-10 overflow-y-auto">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-slate-400 uppercase tracking-widest ml-1">Teléfono (Opcional)</label>
                    <input type="tel" className="w-full px-6 py-4 glass-input rounded-2xl outline-none text-lg font-mono" value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})} placeholder="555-0000" />
                  </div>
                  <div className="space-y-2">
